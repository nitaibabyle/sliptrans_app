...
/*  $Id: solve.QP.compact.f,v 1.13 1998/07/23 05:08:23 bturlach Exp $ */
...
/* Subroutine */ int qpgen1_(doublereal *dmat, doublereal *dvec, integer *
...
/* vector has to be negated (as well as bvec in that case) */
	...
	i__2 = iamat[i__ * iamat_dim1 + 1];
	for (j = 1; j <= i__2; ++j) {
	    sum += amat[j + i__ * amat_dim1] * sol[iamat[j + 1 + i__ * 
		    iamat_dim1]];
/* L61: */
	}
	if (i__ > *meq) {
...
L55:
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	sum = 0.;
	i__2 = iamat[nvl * iamat_dim1 + 1];
	for (j = 1; j <= i__2; ++j) {
	    sum += dmat[iamat[j + 1 + nvl * iamat_dim1] + i__ * dmat_dim1] * 
		    amat[j + nvl * amat_dim1];
/* L81: */
	}
	work[i__] = sum;
/* L80: */
...
/* keep sum (which is z^Tn^+) to update crval below! */
	...
	sum = 0.;
	i__1 = iamat[nvl * iamat_dim1 + 1];
	for (i__ = 1; i__ <= i__1; ++i__) {
	    sum += work[iwzv + iamat[i__ + 1 + nvl * iamat_dim1]] * amat[i__ 
		    + nvl * amat_dim1];
/* L120: */
	}
...
/* the fit violates the chosen constraint now. */
	    ...
	    i__1 = iamat[nvl * iamat_dim1 + 1];
	    for (j = 1; j <= i__1; ++j) {
		sum += sol[iamat[j + 1 + nvl * iamat_dim1]] * amat[j + nvl * 
			amat_dim1];
/* L190: */
	    }
	    if (nvl > *meq) {
